...
OptionsMenu::OptionsMenu(void) {
	...
	this->lMO.push_back(new MenuOption("CAN MOVE BACKWARD", 73, 209));
        this->lMO.push_back(new MenuOption("SPEED OF MINION", 73, 233));
this->lMO.push_back(new MenuOption("MAIN MENU", 73, 265));
...
void OptionsMenu::Draw(SDL_Renderer* rR) {
	...
	CCFG::getText()->Draw(rR, CCFG::canMoveBackward ? "TRUE" : "FALSE", 357, 209, 16, activeMenuOption == 6 ? 255 : 90, activeMenuOption == 6 ? 255 : 90, activeMenuOption == 6 ? 255 : 90);
       CCFG::getText()->Draw(rR, CCFG::getKeyString(CCFG::keyIDSpeed), 357, 233, 16, activeMenuOption == 7 ? 255 : 90, activeMenuOption == 7 ? 255 : 90, activeMenuOption == 7 ? 255 : 90);
	...
	if(inSetKey && keyID != SDLK_KP_ENTER && keyID != SDLK_RETURN && keyID != SDLK_ESCAPE) {
		switch(activeMenuOption) {
			...
			case 5:
				...
				break;
                         case 7:
                                 if(keyID<=79&&keyID>=48)
                                CCFG::keyIDSpeed = std::to_string(keyID-48);
                                 break;
		}
		resetSetKey = true;
